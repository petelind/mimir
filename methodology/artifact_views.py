...
def artifact_create(request, playbook_pk):
    ...
    """
    logger.info(f"User {request.user.username} accessing artifact create for playbook {playbook_pk}")
    ...
    if not playbook.is_owned_by(request.user):
        logger.warning(f"User {request.user.username} attempted to create artifact without permission")
        messages.error(request, "You don't have permission to add artifacts to this playbook.")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logger.info(f"Artifact '{name}' created successfully in playbook {playbook_pk}")
            messages.success(request, f"Artifact '{artifact.name}' created successfully!")
            return redirect('playbook_detail', pk=playbook_pk)
...
def artifact_detail(request, pk):
    ...
    if artifact.playbook.source == 'owned' and artifact.playbook.author != request.user:
        logger.warning(f"User {request.user.username} attempted to access artifact {pk} they don't own")
...
def artifact_edit(request, pk):
    ...
    if not artifact.playbook.is_owned_by(request.user):
        logger.warning(f"User {request.user.username} attempted to edit artifact without permission")
    ...
    if request.method == 'POST':
        ...
        if produced_by_id:
            try:
                produced_by = Activity.objects.get(
                    pk=int(produced_by_id),
                    workflow__playbook=artifact.playbook
                )
