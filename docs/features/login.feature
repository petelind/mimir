...
# Icon: 🔐
# ID Prefix: AUTH
# Test Implementation: tests/e2e/test_auth_login.py (pytest-playwright)
...
## Feature Specification
  ...
  Scenario: AUTH 1.2 Engineer logs out successfully
    ...
    And the session cookie "sessionid" should be cleared
  ...
  @core
  Scenario: AUTH 3.1 Landing page is publicly accessible
    Given no user is logged in
    When the user navigates to "http://localhost:8000/"
    Then the page should display the element with data-testid "methodology-explorer"
    And the page should display the element with data-testid "login-link"
